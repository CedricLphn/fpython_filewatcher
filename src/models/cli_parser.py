...
def cli_parser():
    ...
    if args.logging:
        ...
        handle_log_cases(case='set', filename=_logging)
        logged = True
    ...
    if args.config:
        ...
        if logged is True:
            handle_log_cases(text="Loading YAML config file...", case='info')
    ...
    if args.directory:
        ...
        if logged is True:
            handle_log_cases(text="Setting directory to monitor...", case='info')
        params.append(_directory)
        print("new path to monitor: ", params)
    else:
        if logged is True:
            handle_log_cases(text="Directory to monitor @required", case='critical')
    ...
    if args.interval:
        ...
        if logged is True:
            handle_log_cases(text="Setting interval in seconds...", case='info')
        params.append(_interval)
        print("interval: ", params)
    else:
        if logged is True:
            handle_log_cases(text="Interval in seconds @required", case='critical')
...
def check_params(params, logged):
    if len(params) > 1:
        ...
        if logged is True:
            handle_log_cases(text="Too much critical alerts, stopping to process...", case='critical')
            handle_log_cases(text="Check your logs for more informations", case='debug')
